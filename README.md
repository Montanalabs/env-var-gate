...
- **Closed type:** `type Decision = SetVar(EnvVar) | RejectVar` where `EnvVar` is closed
- **Sink / capability:** `grant setEnv` → `privileged { setEnv(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/env-var-gate/demo.sh
```
...
- `env-var-gate_safe.eide` — the correct design.
- `env-var-gate_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
